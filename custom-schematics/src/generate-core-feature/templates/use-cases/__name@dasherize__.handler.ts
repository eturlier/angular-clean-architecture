 ...
 */
export class <%= classify(name) %>Handler extends AbstractBaseHandler<<%= classify(name) %>, FilterEvent> {
    constructor(protected _<%= camelize(name) %>Loader: <%= classify(name) %>Client) {
        super(_<%= camelize(name) %>Loader);
    }
}
